...
# Use corepack to enable pnpm first in recent Node images
...
# Note: No need to reinstall production dependencies if starting FROM builder,
# as builder already has node_modules. If you started FROM a fresh image,
# you would copy package.json, pnpm-lock.yaml and run pnpm install --prod.
...
COPY --from=builder --chown=node:node /app/public ./public # Copy public folder
